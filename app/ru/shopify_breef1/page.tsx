...
import ShopifyBriefForm from "@/components/shopify-brief-form"
import { Mail, Phone } from "lucide-react" // Import Mail and Phone components
import { useTranslation } from "@/lib/i18n"

export default function ShopifyBriefingPage() {
  const { language, t } = useTranslation()
  const [showConsultationModal, setShowConsultationModal] = useState(false) // Keep for MobileMenu prop
  ...
  return (
    <div className="min-h-screen w-full overflow-x-hidden bg-white">
      ...
      <MobileMenu
        ...
        isLightBackground={true} // Set to true for light background
      />
      ...
      <main className="flex min-h-screen flex-col items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100 py-12 px-4 sm:px-6 lg:px-8">
        <div className="w-full max-w-4xl mx-auto">
          <h1 className="text-4xl font-extrabold text-center text-slate-900 mb-4">{t("shopifyBriefTitle")}</h1>
          <p className="text-lg text-center text-slate-600 mb-12">{t("shopifyBriefSubtitle")}</p>
          <ShopifyBriefForm language={language} t={t} />
        </div>
      </main>
