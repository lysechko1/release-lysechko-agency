...
interface ConsultationModalProps {
  isOpen: boolean
  onClose: (scrollToId?: string) => void
  language: Language
  t: (key: keyof typeof translations) => string
}

export function ConsultationModal({ isOpen, onClose, language, t }: ConsultationModalProps) {
  ...
  useEffect(() => {
    ...
    if (isOpen) {
      ...
      console.log("Consultation modal opened, isSubmitted reset to false.")
    }
  }, [isOpen])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        console.log("Consultation request API call successful!")
        trackFormSubmit("consultation")
        onClose()
      } else {
        const errorData = await response.json()
        console.error("Consultation request API call failed:", errorData)
      }
    } catch (error) {
        ...
        <div className="bg-gradient-to-r from-amber-400 to-orange-500 px-6 py-8 text-center">
          <DialogHeader>
            <DialogTitle className="text-2xl sm:text-3xl font-bold text-black mb-3">
              {isSubmitted ? t("thankYouHeading") : t("modalTitle")}
            </DialogTitle>
            <p className="text-black/80 text-sm sm:text-base leading-relaxed max-w-sm mx-auto">
              {isSubmitted ? t("thankYouSubheading") : t("modalSubtitle")}
            </p>
          </DialogHeader>
        </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                <RadioGroup value={messenger} onValueChange={setMessenger} className="flex space-x-6">
                  <div className="flex items-center space-x-2">
                    <RadioGroupItem value="whatsapp" id="whatsapp" className="border-amber-400 text-amber-400" />
                  ...
                  <div className="flex items-center space-x-2">
                    <RadioGroupItem value="telegram" id="telegram" className="border-amber-400 text-amber-400" />
